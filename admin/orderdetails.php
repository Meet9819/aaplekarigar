...
<div id="wrapper"> 
	...
	<div class="main-content"> 
									<?php
									    ...
									    {
									        $status = $_POST['status'];  
									        $deliveryboy = $_POST['deliveryboy']; $discount = $_POST['discount'];
									        if(!isset($errMSG))
									        {
									        $stmt = $DB_con->prepare('UPDATE payment SET status=:status, deliveryboy =:deliveryboy, discount =:discount
									        WHERE id=:id');
					...
					<table id="example" class="table table-striped table-bordered display" style="width:100%">
						<thead>
							<tr>
								...
								<th >Units </th>
								<th>Sub Total </th>											
								<th>Bill Amt + Shipping CHarges </th> 
								<th>Action</th> 	
								<th>Delete</th> 
							</tr>
						</thead>
						...
						<?php 
							...
							<tr>								  
								...
								<td>&#8377; '.$row['subtotal'].'</td> 
								<td>&#8377; '.$row['finalamount'].' + &#8377; '.$row['shippingcharge'].'</td>							
								<td>'.$row['datee'].' </td>		
								'; ?> <td><span data="<?php echo $row['id'];?>" class="status_checks btn-sm <?php echo ($row['status'])? 'btn-success' : 'btn-danger'?>"><?php echo ($row['status'])? '<i class="fa fa-check" aria-hidden="true"></i>' : '<i class="fa fa-times" aria-hidden="true"></i>'?></span></td>
